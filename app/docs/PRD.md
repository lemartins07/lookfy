...
## 1. Visão geral
Webapp PWA (instalável em celular), mobile-first e acessível via desktop, que atua como consultor de estilo pessoal usando IA para sugerir looks com base no guarda-roupa cadastrado e no estilo desejado. Também recomenda itens a comprar para compor o guarda-roupa.

## 2. Objetivo do MVP
- Gerar looks prontos para ocasiões do dia a dia usando peças já cadastradas.
- Sugerir uma lista de compras complementar (sem integração com lojas).
...
- Lista de compras com prioridades (sem links externos).
...
4. **Completar look**
   - Listar itens sugeridos para compra (priorizados).
5. **Feedback**
...
- Sistema sugere compras complementares.
...
- **Usuário**: estilo alvo, preferências, restrições.
...
- **Lista de compras**: item, prioridade, motivo.
